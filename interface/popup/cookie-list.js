...
(function () {
  ...
  function convertToDict(cookies) {
    ...
    for (const cookieId in cookies) {
      ...
      // sendNotification(typeof exportedCookie);
      exportedCookie.storeId = null;
      if (exportedCookie.sameSite === 'unspecified') {
        exportedCookie.sameSite = null;
      }
      exportedCookies[exportedCookie['name']]=exportedCookie['value'];
    }
    // sendNotification(JSON.stringify(exportedCookies))
  ...
  function exportToDict() {
    ...
    copyText(convertToDict(loadedCookies));

    sendNotification('Cookies exported to clipboard as Dict');
    setTimeout(() => {
